...
module.exports = function performRedirect () {
  return ({ query }: Request, res: Response, next: NextFunction) => {
    ...
    if (security.isRedirectAllowed(toUrl) && security.redirectAllowlist.indexOf(toUrl) !== -1) {
      ...
      res.redirect(security.redirectAllowlist[security.redirectAllowlist.indexOf(toUrl)])
    } else {
      res.status(406)
